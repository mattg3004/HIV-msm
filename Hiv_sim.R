...
generate.hidden.markov.model.data.2.states.plus.death <- function(N, num.data.points, Mean.1, SD.1, Mean.2, SD.2, trans.A.to.B, trans.A.to.Death, trans.B.to.Death, init.proportion.in.A){
  ...
  data = matrix(0, N * num.data.points, 3)
  count = 1
  for(i in 1 : N){
  ...
  colnames(data) = c("ID", "State", "X")
  return(data)
}


results = generate.hidden.markov.model.data.2.states.plus.death(N = 10, num.data.points = 5, Mean.1 = 700, SD.1 = 200, Mean.2 = 400, SD.2 = 200, trans.A.to.B = 0.3, trans.A.to.Death = 0.1, trans.B.to.Death = 0.2, init.proportion.in.A = 0.8)